  ...
  if msg.from.type == 'user' then
    ...
    if msgs > max_msg then
	  ...
	  if msg.to.type == 'user' then
		...
		if msgs >= max_msg then
			print("Pass2")
			send_large_msg("user#id"..msg.from.id, "<i>✨کاربر ["..msg.from.id.."]بخاطر اسپم بلاک شد(ژوپیتر 5.3)✨\n✨برای آزاد سازی فرد به سایت مدیریت کاربران مراجعه کنید: http://SmartTG.Ir✨</i>")
			savelog(msg.from.id.." PM", "<i>✨کاربر ["..msg.from.id.."] بخاطر اسپم بلاک شد(ژوپیتر 5.3)✨\n✨برای آزاد سازی فرد به سایت مدیریت کاربران مراجعه کنید: http://SmartTG.Ir✨</i>")
